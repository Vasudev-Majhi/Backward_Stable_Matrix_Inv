...
class HullKVCache:
    ...
    def insert_v(self, layer, keys, values):
        """Re-insert with updated values for V-cache patching.

        Uses the same key but a higher seq, so TieBreak::LATEST returns the
        new value when future tokens query this position.  The dummy query
        result is discarded.
        """
        self._seq += 1
        self._cache.layer_step(
            layer,
            keys.reshape(-1, 2).numpy(),
            keys.reshape(-1, 2).numpy(),   # dummy query — result not used
            values.reshape(-1, 2).numpy(),
            self._seq,
        )
