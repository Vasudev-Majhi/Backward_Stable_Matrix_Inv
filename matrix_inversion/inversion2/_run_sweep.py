"""Run a build+inference sweep for n=2..30 using the LU pipeline.

Designed to be uploaded and run on the server. Writes a CSV summary.
Forces standard KV cache by default; the Hull C++ cache produces wrong
results on this server (kernel/torch combo issue, see notes).
"""
...
# Must be set BEFORE importing runner_lu so it picks the standard cache.
...
def main(out_csv: str = "lu_sweep.csv", n_min: int = 2, n_max: int = 100,
         model_dir: str = "/tmp/lu_models"):
