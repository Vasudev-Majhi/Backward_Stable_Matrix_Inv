"""Tiny sys.path shim — siblings in this folder import this first.

Adds craft/ (parent) to sys.path so we can import _bootstrap, parse,
interpreter, etc., AND adds craft/rbsor/ so siblings can use plain
`from coloring import ...` style without a package install.
"""
...
# Fallback: if the shared _bootstrap path is stale (transformer-vm moved during
# repo reorgs), try a few known alternate locations before giving up.
