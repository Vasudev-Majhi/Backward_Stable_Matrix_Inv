"""154-circuit sweep for idea3: end-to-end LU inference pipeline.

For each circuit:
  - Builds sidecar (LU model + A_FP) if not cached.
  - Runs inference: b = -A_FP @ V_fixed, then A_FF·x=b via LU transformer.
  - Writes results to cadj_results/results_idea3<suffix>.csv incrementally.
"""
...
def main():
    ...
    ap = argparse.ArgumentParser(description="Idea3: 154-circuit end-to-end LU sweep")
    ...
    for i, c in enumerate(circuits):
        ...
        try:
            # --- build ---
            tb0 = time.time()
            needs_build = args.force_rebuild or not os.path.exists(
                os.path.join(_HERE, "sidecars", cid, "sidecar.json")
            )
            if needs_build:
                ...
                with open(os.path.join(_HERE, "sidecars", cid, "sidecar.json")) as f:
                    info = json.load(f)
                row["N"]       = info.get("N", "")
                row["n_free"]  = info.get("n_free", "")
                row["n_fixed"] = info.get("n_fixed", "")
