...
Inference is inline (does not rely on runner.run_circuit) so it works with
any version of runner.py on the server. Token sequence: start, (skip,
v_init_j)*N, readout, <PRED>, halt — 2N+4 tokens.
"""
...
def run_circuit_idea2(
    ...
    """Run idea2 readout model for cid. Returns (status, pred_v, truth_v, infer_s, n_tokens).

    Inline inference loop — no dependency on runner.run_circuit.
    n_tokens = total forward passes = 2*N + 4 (the transformer's token sequence length).
    """
    import torch
...
def main():
    ...
    print(f"{status}  {args.circuit_id}  pred={pred_v:.4f}  truth={truth:.4f}  "
          f"err={err:.4f}  infer={infer_s:.1f}s  tokens={n_tokens}")
