...
def milp_schedule(
    ...
    # the embedding writes to them. `one` is also protected because the
    # framework's FFN passthrough mechanism (in model/weights.py) creates
    # neurons gated by `one` for every persist's source-slot read and for
    # every erase. Letting `one` die early causes those neurons to fire
    # with relu(0)=0 and silently drop persist contributions.
    protected_dims = {pos, ilp, psq, one_dim}
    alive_sum = {}
    ...
    try:
        ...
        candidate = HiGHS(timeLimit=3600)
    ...
    protected_post = {pos, ilp, psq, one_dim}
    for d in all_dims:
