    ...
    if _shared is not None:
        ...
        for d in input_dims:
            if d not in slot_of:
                slot_of[d] = next_slot
                next_slot += 1
        # `one` is used as the gate of every FFN passthrough/erase neuron,
        # so its slot must never be erased: relu(one) must stay = 1 through
        # every layer or persist contributions get silently zeroed.
        if _one in slot_of:
            protected_slots.add(slot_of[_one])
