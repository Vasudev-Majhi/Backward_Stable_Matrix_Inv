...
public:
        ...
        int seq)
    {
        ...
        for (int h = 0; h < n_heads_; h++) {
            hulls_[base + h].insert(&kp[h * 2], &vp[h * 2], seq);
            hulls_[base + h].query(&qp[h * 2], head_tb_[base + h], &op[h * 2]);
        }
        return result;
    }

    // V-cache patch: rewrite the value of the entry inserted at this seq for
    // every head in this layer, in-place.  values has shape (n_heads, 2).
    void update_v_at_seq(
        int layer,
        int seq,
        py::array_t<double, py::array::c_style | py::array::forcecast> values)
    {
        ...
        int base = layer * n_heads_;
        for (int h = 0; h < n_heads_; h++) {
            hulls_[base + h].update_v_at_seq(seq, &vp[h * 2]);
        }
    }
};

PYBIND11_MODULE(hull_ext, m) {
    ...
    py::class_<HullKVCache>(m, "HullKVCache")
        ...
        .def("set_tiebreak", &HullKVCache::set_tiebreak,
             py::arg("layer"), py::arg("head"), py::arg("latest"))
        .def("layer_step", &HullKVCache::layer_step,
             py::arg("layer"), py::arg("keys"), py::arg("queries"),
             py::arg("values"), py::arg("seq"))
        .def("update_v_at_seq", &HullKVCache::update_v_at_seq,
             py::arg("layer"), py::arg("seq"), py::arg("values"));
}
