 ...
 *
 * Tie-breaking:
 *   AVERAGE returns the mean of all tied values; LATEST returns the value with
 *   the largest seq among the tied entries.  Both are honoured.
 *
 * V-cache patch:
 *   HardAttentionHead::update_v_at_seq(seq, new_val) rewrites the value of an
 *   already-inserted entry (same key, same position) without appending a new
 *   one.  Used by the runner to inject the post-FFN value computed for the
 *   current token before any later token attends to this position.
 */
...
struct _HullCHT {
    struct Line {
        ...
        mutable HullMeta meta;        // mutable so rewrite_meta can update via const iterator
    ...
    // Rewrite vsum / vlast for the line with this exact (m, b), provided it
    // still exists in the envelope (it may have been pruned as dominated, in
    // which case this is a no-op and that's fine: a pruned line cannot win
    // the argmax for any query so its meta is unreachable).
    //
    // Replacement semantics: subtract `old_v` from vsum and add `new_v`; if
    // `last_seq == seq`, overwrite vlast with `new_v`.  count is unchanged.
    bool rewrite_meta(double m_key, double b_key, int seq,
                      const double old_v[2], const double new_v[2]) {
        Line probe;
        probe.m = m_key;
        probe.b = 0.0;
        auto it = lines.lower_bound(probe);
        while (it != lines.end() && it->m == m_key) {
            if (it->b == b_key) {
                // meta is `mutable` on Line; the multiset's ordering uses only
                // m, so mutating meta cannot invalidate the iterator.
                it->meta.vsum[0] += new_v[0] - old_v[0];
                it->meta.vsum[1] += new_v[1] - old_v[1];
                if (it->meta.last_seq == seq) {
                    it->meta.vlast[0] = new_v[0];
                    it->meta.vlast[1] = new_v[1];
                }
                return true;
            }
            ++it;
...
struct HullHalf {
    ...
    // ─── Rewrite (V-cache patch) ─────────────────────────────────────
    // Find the envelope line for this (kx, ky) and rewrite its meta in-place,
    // replacing old_v's contribution with new_v.  Returns true if found.
    bool rewrite(double kx, double ky, int seq,
                 const double old_v[2], const double new_v[2]) {
        if (is_upper) {
            return cht.rewrite_meta(kx, ky, seq, old_v, new_v);
        }
        return cht.rewrite_meta(-kx, -ky, seq, old_v, new_v);
    }
...
struct HardAttentionHead {
    ...
    int      n = 0;
    // Maps seq -> (kx, ky, vx, vy) for the entry inserted at that seq.
    // Used by update_v_at_seq for the V-cache patch.
    std::unordered_map<int, EntrySnapshot> seq_to_entry;
    ...
    void insert(const double key[2], const double val[2], int seq = 0) {
        ...
        seq_to_entry[seq] = EntrySnapshot{key[0], key[1], val[0], val[1]};
        n++;
    }

    // Replace V at the entry inserted for this seq.  Same K, no append.
    // No-op if seq is unknown (e.g. clear() was called or seq was never
    // inserted).  vsum / vlast are rewritten on every aggregate that this
    // entry contributed to; count is unchanged.
    bool update_v_at_seq(int seq, const double new_val[2]) {
        auto it = seq_to_entry.find(seq);
        if (it == seq_to_entry.end()) return false;
        EntrySnapshot& e = it->second;
        const double old_v[2] = {e.vx, e.vy};

        auto rewrite_aggregate = [&](HullMeta& m) {
            m.vsum[0] += new_val[0] - old_v[0];
            m.vsum[1] += new_val[1] - old_v[1];
            if (m.last_seq == seq) {
                m.vlast[0] = new_val[0];
                m.vlast[1] = new_val[1];
            }
        ...
        rewrite_aggregate(global);
        if (e.kx == min_kx) rewrite_aggregate(left_meta);
        if (e.kx == max_kx) rewrite_aggregate(right_meta);

        upper.rewrite(e.kx, e.ky, seq, old_v, new_val);
        lower.rewrite(e.kx, e.ky, seq, old_v, new_val);
...
struct BruteAttentionHead {
    ...
    // V-cache patch parity with HardAttentionHead.
    bool update_v_at_seq(int seq, const double new_val[2]) {
        for (auto& e : entries) {
            if (e.seq == seq) {
                e.vx = new_val[0];
